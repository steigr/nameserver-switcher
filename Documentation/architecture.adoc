...
package "External" {
  [CoreDNS] as CoreDNS
  [System DNS] as SysDNS
  [Explicit DNS] as ExpDNS
  [Prometheus] as Prom
}

CoreDNS --> DNS : forward plugin
CoreDNS --> GRPC : grpc plugin
Router --> SysDNS : fallback queries
Router --> ExpDNS : matched queries
HTTP --> Prom : /metrics
@enduml
....
...
* **API**: Defined in `pkg/api/v1/switcher.proto`
